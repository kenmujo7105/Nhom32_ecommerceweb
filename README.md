...
- **VNPay** (Tích hợp cổng thanh toán trực tuyến nội địa)
- **Nodemailer** (Hệ thống gửi email tự động)
- **Google Gemini API** (Tích hợp Chatbot AI tư vấn tự động)
...
- **Xác thực & Phân quyền**: Đăng ký, Đăng nhập, và tính năng **Quên mật khẩu** (Xác thực qua mã OTP gửi tới Email). Hỗ trợ đăng nhập nhanh bằng **Google**.
- **Trang chủ & Sản phẩm**: Xem danh sách sản phẩm (hỗ trợ Tiếng Việt), lọc và tìm kiếm, xem chi tiết sản phẩm.
- **Đánh giá Sản phẩm (Reviews)**: Viết đánh giá (rating sao & bình luận) dành cho các khách hàng đã hoàn thành đơn hàng.
- **AI Chatbot**: Tích hợp trợ lý ảo thông minh giải đáp thắc mắc người dùng ngay trên giao diện web.
- **Giỏ hàng & Thanh toán**: Thêm sản phẩm vào giỏ, đặt hàng, tích hợp thanh toán an toàn qua cổng **VNPay**.
- **Quản lý tài khoản**: Xem lịch sử mua hàng, cập nhật thông tin cá nhân và thay đổi mật khẩu.

### Dành cho Quản trị viên (Admin)
- **Dashboard Thống kê**: Xem tổng quan doanh thu, số lượng đơn hàng, và người dùng mới.
- **Quản lý Danh mục (Categories)**: Thêm, sửa, xoá danh mục. Theo dõi **Tổng tồn kho** trực tiếp trên từng danh mục.
- **Quản lý Sản phẩm (Products)**: Thêm sản phẩm kèm nhiều ảnh (Gallery Images), quản lý giá và số lượng tồn kho.
- **Quản lý Đơn hàng (Orders)**: Xem và cập nhật trạng thái đơn hàng (Đang xử lý, Đang giao, Đã giao...).
- **Quản lý Người dùng (Users)**: Theo dõi khách hàng, kích hoạt/vô hiệu hóa tài khoản.
...
VNP_RETURNURL=http://localhost:5173/order-success/{orderId}
...
5. Trong mục **Variables** của Railway, thêm thông tin kết nối do MySQL plugin cung cấp và cấu hình tất cả biến môi trường khác (`JWT_SECRET`, `SMTP_*`, `VNP_*`).
