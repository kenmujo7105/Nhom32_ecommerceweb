...
import { Search, Shield, ShieldAlert, Power, User } from 'lucide-react';
...
const Users = () => {
  ...
  const columns = [
    ...
    { header: 'Joined', render: (row) => <span className="text-slate-600">{new Date(row.createdAt || Date.now()).toLocaleDateString()}</span> },
    { 
      header: 'Actions', 
      ...
      render: (row) => (
        <div className="flex items-center justify-end gap-2">
          <button 
            onClick={() => handleActionClick('role', row)}
            className="px-2.5 py-1.5 text-xs font-medium bg-white border border-slate-300 text-slate-700 hover:bg-slate-50 rounded-md transition-colors"
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-4 rounded-xl shadow-sm border border-slate-200">
        <h2 className="text-lg font-semibold text-slate-800 hidden md:block">Users</h2>
        ...
        <div className="relative w-full sm:w-auto">
          ...
          <input 
            ...
            className="pl-10 pr-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 w-full sm:w-72 text-sm outline-none"
          />
        </div>
      </div>
